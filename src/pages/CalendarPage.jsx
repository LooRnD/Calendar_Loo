...
function EventModal({ isOpen, onClose, event = null, defaultDate = null }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={event ? 'Sửa Sự Kiện' : 'Thêm Sự Kiện'}>
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          <label className="form-label">Tên sự kiện *</label>
          <input className="form-input" value={form.title}
            onChange={e => set('title', e.target.value)}
            placeholder="Tên sự kiện..." required autoFocus maxLength={200} />
        </div>
        <div className="form-group">
        ...
        <div className="form-group flex items-center gap-8">
          ...
          <label htmlFor="all-day" className="form-label" style={{ marginBottom: 0 }}>Cả ngày</label>
        </div>
        <div className="form-group">
    ...
    <div className="page-enter" style={{ display: 'flex', gap: 0, height: '100%' }}>
        ...
        <div className="section-header mb-20">
          <div className="flex items-center gap-12">
            ...
            <div className="flex gap-4">
              <button className="btn-icon" onClick={() => setCurrentDate(c => c.subtract(1, 'month'))} aria-label="Tháng trước">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <polyline points="15 18 9 12 15 6"/>
                </svg>
              </button>
              <button className="btn btn-ghost" onClick={() => setCurrentDate(dayjs())} style={{ padding: '6px 12px', fontSize: 12 }}>Hôm nay</button>
              <button className="btn-icon" onClick={() => setCurrentDate(c => c.add(1, 'month'))} aria-label="Tháng sau">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <div style={{ width: 260, borderLeft: '1px solid var(--border)', paddingLeft: 20, overflowY: 'auto' }}>
        <div className="flex items-center justify-between mb-16">
          <h2 className="fs-16 fw-700 text-primary">Lịch sắp tới</h2>
        ...
        {Object.keys(grouped).length === 0 ? (
          <div style={{ textAlign: 'center', padding: '30px 0', color: 'var(--text-muted)' }}>
            ...
            <div className="fs-13">Không có sự kiện sắp tới</div>
          </div>
        ) : (
              ...
              {evts.map(ev => (
                <div key={ev.id} className="calendar-event-item"
                  ...
                  <div className="event-time">
                    {ev.allDay ? 'Cả ngày' : dayjs(ev.startDate).format('HH:mm')}
                  </div>
